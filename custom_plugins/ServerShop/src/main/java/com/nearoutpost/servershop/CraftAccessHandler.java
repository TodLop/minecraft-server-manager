...
/**
 * Handles the purchase flow for crafting table access.
 */
public class CraftAccessHandler {

    private final ServerShop plugin;

    public CraftAccessHandler(ServerShop plugin) {
        this.plugin = plugin;
    }

    /**
     * Process crafting table access purchase for a player.
     *
    ...
    public boolean purchase(Player player) {
        ...
        if (!plugin.getConfig().getBoolean("craft_access.enabled", true)) {
            player.sendMessage(colorize(getMessage("craft-feature-disabled")));
            return true;
        }

        // Get permission node from config
        String permission = plugin.getConfig().getString("craft_access.permission", "servershop.craft.use");

        // Check if player already has permission
        if (player.hasPermission(permission)) {
            player.sendMessage(colorize(getMessage("craft-already-owned")));
            return true;
        }
        ...
        // Success!
        player.sendMessage(colorize(getMessage("craft-purchased")
                .replace("%price%", String.format("%.0f", (double) price))));
    ...
    /**
     * Show information about crafting table access purchase.
     *
    ...
    public boolean showInfo(Player player) {
        ...
        player.sendMessage(colorize(getMessage("craft-price-info")
                .replace("%price%", String.format("%.0f", (double) price))));
